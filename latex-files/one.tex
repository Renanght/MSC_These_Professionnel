...
L'intérêt du cas dépasse sa sophistication, qui est faible. Il montre que confier à un modèle l'évaluation d'un contenu non fiable revient à hériter du problème, non résolu, de l'injection de prompt indirecte. Pour un travail dont la contribution est défensive et fondée sur l'IA, c'est une contrainte de conception, pas une remarque annexe.
...
Le déséquilibre entre le coût de production d'une variante et celui de son analyse est souvent avancé. Il est réel, mais il n'est pas propre à l'IA : les services de reconditionnement évoqués en section \ref{sec:economie} produisent déjà cet effet. L'apport des modèles est d'abaisser encore le seuil d'entrée et de supprimer la dépendance à un fournisseur.

\begin{quote}
\small\itshape
Note de rédaction : étayer par un coût par requête relevé sur une grille tarifaire publique et une estimation du temps d'analyse issue de la littérature, plutôt que de laisser ce point à l'état d'assertion.
\end{quote}
...
Ces travaux fixent la condition de recevabilité d'une contribution défensive fondée sur l'IA : un dispositif évalué uniquement sur des échantillons non adverses ne démontre rien sur sa robustesse.
...
Le verrou traité dans la suite de ce mémoire se formule donc ainsi : \emph{comment concevoir, pour des binaires ELF, un dispositif de détection dont le coût d'adaptation à une variation adverse ne croisse pas plus vite que le coût de production de cette variation, et l'évaluer sous des contraintes opérationnelles réalistes, à savoir un taux de faux positifs compatible avec l'exploitation d'un parc et une robustesse mesurée face à des échantillons adverses ?}

\begin{quote}
\small\itshape
Note de rédaction : cette formulation est une proposition, à aligner sur la contribution effective du mémoire.
\end{quote}